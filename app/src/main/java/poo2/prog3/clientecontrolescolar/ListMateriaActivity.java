...
public class ListMateriaActivity extends AppCompatActivity {
    private List<Materia> mDatos;
    private RecyclerView recyclerView;
    private DaoEscolaresImpl mDao;
    private View mLayout;
    private ProgressBar progressBar;
    private MateriaRecyclerViewAdapter adapterMateria;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_list_materia);
        mDao = new DaoEscolaresImpl(getString(R.string.url_base));
        mLayout=findViewById(R.id.contenedor_lista_materia);
        ...
        FloatingActionButton fabNueva = findViewById(R.id.fab_nueva_materia);
        ManejadorNuevaMateria manNueva = new ManejadorNuevaMateria();
        ...
        recyclerView = findViewById(R.id.materia_list);
     ...
     * al Floating Action Button para agregar una nueva Materia
     */
    private class ManejadorNuevaMateria implements View.OnClickListener {

        @Override
        public void onClick(View v) {
            Intent intent = new Intent(ListMateriaActivity.this, DetalleMateriaActivity.class);
    ...
    /**
     * Este metodo genera datos de manera manual para mostrar en la lista
     */
    private void generaDatos() {
        mDatos =new ArrayList<>();
        Materia m = new Materia("CS201","Prog. Orientada a Obj. I",2,"INGSW2010");
        mDatos.add(m);
        m=new Materia("CS202","Lab. de Prog. Orientada a Obj. I",2,"INGSW2010");
        mDatos.add(m);
        m=new Materia("IC501","Sistemas Operativos",5,"COMP2005");
        mDatos.add(m);
        m=new Materia("IC601","Compiladores",6,"COMP2005");
        mDatos.add(m);
    }
    ...
    private void llenaRecyclerView() {
        ...
        // TODO Cambiar para obtener los datos del Servicio REST
        generaDatos();
        adapterMateria = new MateriaRecyclerViewAdapter(this, mDatos);
        recyclerView.setAdapter(adapterMateria);
        progressBar.setVisibility(View.GONE);
    }

    /**
     * Clase para crear el adaptador con los datos a mostrarse en el RecyclerView de Materias
     */
    public static class MateriaRecyclerViewAdapter
            extends RecyclerView.Adapter<MateriaRecyclerViewAdapter.ViewHolder> {
        // Almacenamos una referencia a la Activity especifica en la que esta el RecyclerView
        private final ListMateriaActivity mParentActivity;
        ...
        private final View.OnClickListener mOnClickListener = new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Materia item = (Materia) view.getTag();
                    Intent intent = new Intent(mParentActivity, DetalleMateriaActivity.class);
                    intent.putExtra("materia", item);
         ...
         */
        MateriaRecyclerViewAdapter(ListMateriaActivity parent,
                                   List<Materia> items) {
            mValues = items;
        ...
        public void onBindViewHolder(final ViewHolder holder, int position) {
            // Para el caso de Materia, el layout contiene campos para la
            // clave y el nombre
            holder.mIdView.setText(mValues.get(position).getClaveMateria());
            holder.mContentView.setText(mValues.get(position).getNombreMateria());

            // Le asignamos como tag a este View el objeto Materia correspondiente
        ...
        /**
         * Clase interna que representa al ViewHolder de la Materia a mostrar
         */
    ...
    protected void onActivityResult(int requestCode,
        ...
        if (resultCode==RESULT_OK) {
            ...
            Materia materia = (Materia) datos.getSerializable("materia");
            if (requestCode == SOLICITUD_NUEVA) {
                mDatos.add(materia);
                adapterMateria.notifyItemInserted(mDatos.size()-1);
            }
            else {
                int operacion = datos.getInt("operacion");
                int pos = mDatos.indexOf(materia);
                if (operacion == OPERACION_ELIMINACION) {
                    mDatos.remove(pos);
                    adapterMateria.notifyItemRemoved(pos);
                }
                else {
                    mDatos.set(pos,materia);
                    adapterMateria.notifyItemChanged(pos);
                }
            }
    ...
    protected void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        outState.putSerializable("datos", (ArrayList<Materia>) mDatos);
    }
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        mDatos= (ArrayList<Materia>) savedInstanceState.getSerializable("datos");
        if (mDatos != null) {
            adapterMateria = new MateriaRecyclerViewAdapter(this,mDatos);
            recyclerView.setAdapter(adapterMateria);
        } else {
